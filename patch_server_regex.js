...
// Chunk 1 replacement
const target1 = `                // 1. Buscar en call_center_prospectos
...
const target1Lines = target1.split('\n').map(l => l.trim()).join('\\s*');
const regex1 = new RegExp(target1Lines.replace(/[.*+?^${}()|[\]\\]/g, '\\$&').replace(/\\s\\\*/g, '\\s*'));

const replacement1 = `                // 1. Buscar en call_center_prospectos
                const { data: leads } = await supabase.from('call_center_prospectos').select('id, telefono, historial_llamadas');
...
if (regex1.test(code)) {
    ...
    console.log("Replaced block 1 successfully via regex!");
} else {
    // Let's do string replacement fallback
    code = code.replace(target1, replacement1);
    console.log("Tried string replacement 1");
}

const target2 = `                        // 1. Intentar guardar en leads
...
const target2Lines = target2.split('\n').map(l => l.trim()).join('\\s*');
const regex2 = new RegExp(target2Lines.replace(/[.*+?^${}()|[\]\\]/g, '\\$&').replace(/\\s\\\*/g, '\\s*'));

const replacement2 = `                        // 1. Intentar guardar en leads
                        const { data: leads } = await supabase.from('call_center_prospectos').select('id, historial_llamadas');
                        if (leads) {
                            for (const lead of leads) {
                                if (!lead.historial_llamadas) continue;
...
if (regex2.test(code)) {
    ...
    console.log("Replaced block 2 successfully via regex!");
} else {
    code = code.replace(target2, replacement2);
    console.log("Tried string replacement 2");
}
