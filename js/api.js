...
async function _syncReceiptToClient(projectId, url, label, forceTipo = null) {
  try {
    ...
    const fallbackVendedor = cli.vendedor_asignado_id || proy.asignado_a || proy.responsable_id || (dynAsignado ? dynAsignado.valor : '');
...
export function isProjectFinished(p, db) {
  ...
  if (db && db.Admin_Fases) {
    ...
    if (faseObj) {
      ...
      // Lógica específica para SOLAR: contar como completado (venta) desde la fase 2 en adelante
      if (db.Admin_Pipelines) {
        ...
        if (pipeline && String(pipeline.nombre || '').toLowerCase().includes('solar')) {
          if (typeof faseObj.orden === 'number' && faseObj.orden >= 2) {
             return true;
          }
        }
      }
