      ...
      try {
          ...
          if (workers.length === 0) {
              ...
              container.innerHTML = workers.map(w => {
                  ...
                  if (!isChecked && props.colaboradores && Array.isArray(props.colaboradores)) {
...
async function renderListaPreciosAdmin() {
  ...
  const rowsHtml = allProducts.map(p => {
    ...
    const searchData = (p.nombre + ' ' + (p.codigo||'') + ' ' + (p.categoria||'')).toLowerCase();
    let isHidden = '';
    if (state.preciosSearchQuery && !searchData.includes(state.preciosSearchQuery.toLowerCase())) {
        isHidden = 'display:none;';
    }

    return `
    <tr class="border-b border-gray-100 dark:border-white/5 hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors group precios-table-row" data-search="${searchData}" style="${isHidden}">
      <td class="px-6 py-4 whitespace-nowrap">
  ...
  UI.canvas.innerHTML = `
    <div class="max-w-7xl mx-auto animate-fadeIn">
      <div class="flex flex-col xl:flex-row justify-between xl:items-center mb-6 gap-6">
          ...
          <div class="flex flex-col md:flex-row items-center justify-end gap-3 w-full xl:w-auto mt-4 xl:mt-0 flex-1">
              ...
              ${isAdminPriceList ? `
              <button onclick="window.syncGoogleSheets()" id="btn-sync-sheets" class="flex-shrink-0 flex items-center justify-center gap-2 px-6 py-3 bg-green-500/10 border border-green-500/20 rounded-xl text-green-600 text-[10px] font-black uppercase tracking-widest hover:bg-green-500 hover:text-white transition-all">
...
window.setPreciosSearch = function(query) {
    ...
    const rows = document.querySelectorAll('.precios-table-row');
    rows.forEach(row => {
...
window.openChatAccessModal = async function(cliente_id) {
    ...
    const html = `
    <div id="modal-cli-chat-access" class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4" style="z-index: 2147483647 !important;">
