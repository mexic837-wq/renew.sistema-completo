...
async function buildDetailView(screen, deal, pipeline, fases, curFidx, db, respuestas = []) {
  ...
  screen.innerHTML = `
    ...
    <div style="display:flex; flex-wrap:wrap; gap:56px; padding: 24px 40px; padding-bottom: 40px; max-width: 100%; margin: 0 auto; width: 100%;">
      <!-- Left Column -->
      <div style="flex: 0 0 480px; display:flex; flex-direction:column; gap:24px; max-width: 100%;">
                  ...
                  <div class="obs-item" data-id="${w.id}" style="display:flex;align-items:center;justify-content:space-between;padding:12px;background:var(--surface-alt);border:1px solid var(--border);border-radius:12px;cursor:pointer;">
                      <div style="display:flex;align-items:center;gap:12px;">
                          ...
                          <div>
                              ...
                              <div style="font-size:0.7rem;color:var(--text-muted);">${window.getUserRoles(w).join(', ')}</div>
                          </div>
                      </div>
...
async function renderDynamicAction(deal, pipeline, fases, curFidx, db) {
  ...
  const inputsHtml = campos.map(c => {
    ...
    if(c.tipo === 'Desplegable' || isFinanciera) {
       ...
       const technicians = allUsers.filter(w => window.getUserRoles(w).some(r => /t[eé]cn[io]co/i.test(r)));
...
window._abrirReciboModal = async function(campoId, tipo, dealId) {
  ...
  const tecnicoAsignado = (db.Usuarios || []).find(u => window.getUserRoles(u).some(r => /t[eé]cn[io]co/i.test(r)) && (u.id === deal.tecnico_id || u.id === deal.asignado_a));
