...
export function _renderToolsForPipeline(user, activeUnit) {
  ...
  const TOOLS = {
    'Renew Water': [
      ...
      ((user.permisos && 'app_precios' in user.permisos) ? user.permisos.app_precios : !(userRole.includes('call') || /t[eé]cn[io]co/i.test(userRole) || userRole.includes('representante') || userRole.includes('vendedor') || userRole.includes('supervisor') || userRole.includes('supervisión') || userRole.includes('manager'))) ? {
