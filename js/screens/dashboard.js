...
export const RANK_CONFIG = {
  'Renew Water': [
    ...
    { name: 'Vendedor',            emoji: '<i class="fa-solid fa-bolt"></i>',      color: '#1e40af', bg: 'rgba(30,64,175,0.1)',  border: 'rgba(30,64,175,0.25)', minSales: 53, type: 'total', priceKey: 'precio_vendedor' },
    ...
    { name: 'Oficina', emoji: '<i class="fa-solid fa-star"></i>',      color: '#f59e0b', bg: 'rgba(245,158,11,0.1)', border: 'rgba(245,158,11,0.25)', minSales: 15, type: 'monthly', priceKey: 'precio_oficina' },
  ]
};
...
async function initRendimientoChart(user) {
  ...
  recibos.forEach(r => {
    if (String(r.trabajador_id) !== String(user.id)) return;
    let pDate = null;
    let rawDate = r.fecha_recibo || r.created_at;
    if (rawDate) {
    ...
    if (pDate) {
      ...
      if (isMatch) {
          ...
          const amt = parseFloat(json.grand_total) || parseFloat(json.total_price) || 0;
          comisionesEstimadas += amt;
      }
    }
  });
      ...
      quickStatsEl.innerHTML = `
        ...
        <div style="display:flex; gap:12px">
          ...
          <div style="flex:1; background:rgba(255,255,255,0.03); backdrop-filter: blur(10px); padding:20px; border-radius:20px; border:1px solid rgba(255,255,255,0.08); box-shadow: 0 8px 32px rgba(0,0,0,0.2);">
            ...
            <p style="font-size:1.8rem; font-weight:800; color:#3b82f6; margin:0">$${comisionesEstimadas.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</p>
  ...
  if (rankBannerRend) {
    if (isStrictTecnico || isCallCenter) {
  ...
  window.rendimientoChartInstance = new Chart(ctx.getContext('2d'), {
    type: 'doughnut',
    data: {
      labels: [isStrictTecnico ? 'Completadas' : 'Ventas', isStrictTecnico ? 'Pendientes' : 'En Proceso'],
      datasets: [
        {
...
async function initLeaderboardChart(user) {
  ...
  podiumOrder.forEach(idx => {
    ...
    const m = medals[idx];
    const rank = idx + 1;
    const salesLabel = isStrictTecnico ? 'Citas' : (isCallCenter ? 'Leads' : t('lb_sales'));
    const hiddenLabel = isStrictTecnico ? 'Citas Ocultas' : (isCallCenter ? 'Leads Ocultos' : 'Ventas Ocultas');
  ...
  tab.innerHTML = `
    <div style="margin-bottom:20px;">
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px; flex-wrap:wrap; gap:10px;">
        <h2 style="font-size:1.2rem;font-weight:900;color:var(--text-primary);margin:0;">\uD83C\uDFC6 ${isStrictTecnico ? 'Tabla de Posiciones' : t('lb_title')}</h2>
      </div>
