/* ============================================================
   RENEW OS â€“ screens/catalogo.js
   Pantalla "CatÃ¡logo" (PresentaciÃ³n a clientes)
   ============================================================ */
...
export async function renderCatalogo() {
  const screen = document.getElementById('screen-catalogo');
  if (!screen) return;
  ...
  if (!isAdmin && !isVentas && user.ver_catalogo !== true && user.ver_catalogo !== 'true') {
    ...
    if (rankInfo && rankInfo.isNoAplica) {
      screen.innerHTML = `
        <div class="dash-header" style="padding-bottom:12px;">
          <div class="dash-header-top" style="display:flex; align-items:center; justify-content:center; position:relative; min-height:60px;">
            ...
            <div class="dash-greeting" style="text-align:center;">
              <h1 style="margin:0; font-size:1.3rem;">CatÃ¡logo</h1>
            </div>
          </div>
        </div>
        <div style="padding: 60px 20px; text-align: center; color: var(--text-muted);">
          ...
          <p style="font-size: 0.85rem; opacity: 0.8; margin-top: 8px;">No tienes acceso al catÃ¡logo en este momento. Por favor, contacta a administraciÃ³n si crees que esto es un error.</p>
        </div>
      `;
      setTimeout(() => {
  ...
  screen.innerHTML = `
    <div class="dash-header" style="padding-bottom:12px;">
      <div class="dash-header-top" style="display:flex; align-items:center; justify-content:center; position:relative; min-height:60px;">
        ...
        <div class="dash-greeting" style="text-align:center;">
          ...
          <h1 style="margin:0; font-size:1.3rem;">CatÃ¡logo</h1>
          <p style="font-size:0.75rem; color:var(--text-muted); margin:4px 0 0;">Usa esto para presentar opciones a tus clientes</p>
        </div>
      </div>
    </div>
    ...
    <div style="padding:12px 16px 0;">
      <div style="position:relative; margin-bottom:12px;">
        ...
        <input type="text" id="catalogo-search" placeholder="Buscar producto o cÃ³digo..."
...
function _showProductDetail(prod, priceKey, isAdmin) {
  ...
  const ranksHTML = `
    ...
    <div style="margin-top:20px; background:linear-gradient(135deg,#0ea5e915,#0284c705); border:1.5px solid #0ea5e930; border-radius:18px; padding:24px; text-align:center;">
      ...
      <div id="catalogo-precio-error" style="display:none; color:#ef4444; font-size:0.8rem; font-weight:700; margin-top:12px; background:#ef444415; padding:8px; border-radius:8px;">
        <i class="fas fa-exclamation-circle"></i> Precio fuera de rango permitido.
      </div>
      
      ${prod.unidad ? `<p style="font-size:0.75rem; color:var(--text-muted); margin:12px 0 0; font-weight:600;">precio por ${prod.unidad}</p>` : ''}
    </div>
  `;
  ...
  // Logic for the interactive price input
  const inputEl = modal.querySelector('#catalogo-precio-input');
  const errorEl = modal.querySelector('#catalogo-precio-error');
  
  if (inputEl && (prod.precio_minimo || prod.precio_maximo)) {
    inputEl.addEventListener('input', () => {
      const val = parseFloat(inputEl.value) || 0;
      let isInvalid = false;
      if (prod.precio_minimo && val < prod.precio_minimo) isInvalid = true;
      if (prod.precio_maximo && val > prod.precio_maximo) isInvalid = true;
