    ...
    main.innerHTML = `
        <div class="px-10 py-10 fade-in w-full max-w-[1600px] mx-auto min-h-screen flex flex-col">
            <div class="flex items-center justify-between mb-8">
                <div>
                    <div class="aqua-label"><i class="fa-solid fa-headset mr-2"></i> M贸dulo Call Center</div>
                    <h2 class="text-4xl font-black text-gray-900 dark:text-white tracking-tighter leading-none mb-2">Call Center Leads</h2>
                    <p class="text-[12px] uppercase tracking-[0.2em] font-bold text-gray-500">Gesti贸n de prospectos del Call Center y carga manual</p>
                </div>
                <button id="btn-add-cc-lead" class="btn-premium">
                    <i class="fa-solid fa-user-plus text-lg"></i> A帽adir Lead Call Center
                </button>
            </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="border-b border-gray-100 dark:border-white/10 text-[10px] uppercase tracking-[0.15em] text-gray-400 dark:text-gray-500">
                                ...
                                <th class="pb-4 font-black">Tel茅fono</th>
                                <th class="pb-4 font-black">Direcci贸n</th>
                                ...
                                <th class="pb-4 font-black text-right">Acci贸n</th>
                            </tr>
                        </thead>
            ...
            <div class="bg-white dark:bg-[#111827] w-full max-w-2xl rounded-3xl shadow-2xl border border-gray-100 dark:border-white/10 overflow-hidden transform scale-95 transition-all duration-300 p-8">
                ...
                <form id="form-cc-admin" class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-1">Tel茅fono *</label>
                    ...
                    <div>
                        <label class="block text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-1">Direcci贸n Completa *</label>
                    ...
                    <div>
                        ...
                        <select id="sel-cc-operador" class="w-full bg-gray-50 dark:bg-[#0B0F1A] border border-gray-200 dark:border-white/5 rounded-xl px-4 py-3 text-sm text-gray-800 dark:text-white focus:border-tealAccent focus:ring-1 focus:ring-tealAccent outline-none transition-all">
                            <option value="">Selecci贸n Autom谩tica</option>
                        </select>
                    </div>
...
async function loadOperators() {
    ...
    try {
        ...
        sel.innerHTML = '<option value="">Selecci贸n Autom谩tica</option>';
...
window.adminDeleteCCLead = async (id) => {
    if (!confirm('驴Eliminar este lead permanentemente?')) return;
    try {
...
window.adminZadarmaCall = async (phone) => {
    ...
    if (!confirm('緿eseas llamar al ' + phone + ' desde tu extensi髇 ' + currentUser.zadarma_sip_id + '?')) return;
    ...
    try {
        ...
        if (data.status === 'success') {
            showToast('Llamada en curso. Contesta tu tel閒ono Zadarma.', 'success');
        } else {
