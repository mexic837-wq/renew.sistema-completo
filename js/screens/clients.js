...
function resetModal() {
  ...
  const scanBtn = document.getElementById('btn-quick-scan-id');
  if (scanBtn) {
    scanBtn.style.background = 'rgba(0, 245, 212, 0.05)';
...
function _wireModalControls(user, container) {
  ...
  // ── Smart ID Scanner ──────────────────────────────────────
  const scanBtn = document.getElementById('btn-quick-scan-id');
  const scanInp = document.getElementById('inp-quick-scan-id');
  ...
  if (scanBtn && scanInp) {
    const freshInp = scanInp.cloneNode(true);
    scanInp.parentNode.replaceChild(freshInp, scanInp);
    
    scanBtn.addEventListener('click', () => freshInp.click());
    freshInp.addEventListener('change', async () => {
      const file = freshInp.files[0];
      if (!file) return;
      ...
      try {
        ...
        scanBtn.style.borderColor = 'var(--primary)';
        scanBtn.style.background = 'rgba(0, 245, 212, 0.1)';

        // 2. Upload file (maintain original save logic)
        showToast('Subiendo ID...', 'info');
        const fileUrl = await uploadFile(file, 'clients');
        quickAdjID = fileUrl; 
        ...
        if (progressText) progressText.textContent = 'Analizando ID...';
        const { data: { text } } = await Tesseract.recognize(file, 'eng+spa', {
          logger: m => {
            if (m.status === 'recognizing text' && progressText) {
              progressText.textContent = `Procesando IA (${Math.round(m.progress * 100)}%)`;
            }
          }
        });
        ...
        showToast('Error al escanear ID', 'error');
      } finally {
        if (idleState) idleState.style.display = 'block';
        if (loadingState) loadingState.style.display = 'none';
        
        // Also update the hidden original button state if needed
        const oldLbl = document.getElementById('lbl-adj-id');
        if (oldLbl) { oldLbl.textContent = '✓ ESCANEADO'; oldLbl.style.color = 'var(--primary)'; }
      }
    });
  }
