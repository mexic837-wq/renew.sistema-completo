  ...
  if (btnDecline) {
    btnDecline.addEventListener('click', async () => {
      const { value: motivo } = await Swal.fire({
        ...
        customClass: {
          popup: 'rounded-[24px] border border-[var(--border)] shadow-2xl',
          ...
          if (window.quickSelectedClientId) {
             ...
             if (isCita && citaDate) {
                 ...
                 const nuevoEvento = {
                     ...
                     fecha_fin: citaDate,
                     direccion: dir,
                     descripcion: `Cita con prospecto. Tel: ${tel || 'N/A'}\nNotas: ${notas || ''}`.trim(),
                     color: 'Verde',
                     colaboradores: [{ id: user.id, nombre: `${user.nombre} ${user.apellido || ''}`.trim(), email: user.email || '' }],
                     departamentos: [dept],
                     attendees: [],
