...
// Loose regex to find the point where things went wrong
const badRegex = /isWorkOrder\s*=\s*\(type\s*===\s*'WORK_ORDER_SUBMITTED'\);\s*window\._verRecibosWorker/m;
...
window._verRecibosWorker`;

if (badRegex.test(content)) {
    content = content.replace(badRegex, goodLines);
    fs.writeFileSync(path, content, 'utf8');
    console.log('Fixed with loose regex');
} else {
    console.log('STILL could not find the mangled line');
    // Let's print a small chunk to see what's actually there
    const index = content.indexOf('isWorkOrder');
    console.log('Snippet around isWorkOrder:', content.substring(index, index + 200));
}
